...
class TestQueueFunctions(testutils.WebHostTestCase):
    ...
    def test_queue_basic(self):
        r = self.webhost.request("POST", "put_queue", data="test-message")
    ...
    def test_queue_return(self):
        r = self.webhost.request("POST", "put_queue_return", data="test-message-return")
        self.assertEqual(r.status_code, 200)
    ...
    def test_queue_message_object_return(self):
        r = self.webhost.request(
            "POST", "put_queue_message_return", data="test-message-object-return"
        )
        self.assertEqual(r.status_code, 200)
    ...
    def test_queue_untyped_return(self):
        r = self.webhost.request(
            "POST", "put_queue_untyped_return", data="test-untyped-return"
        )
        self.assertEqual(r.status_code, 200)
    ...
    def test_queue_return_multiple(self):
        r = self.webhost.request("POST", "put_queue_return_multiple", data="foo")
        self.assertTrue(
            200 <= r.status_code < 300,
            f"Returned status code {r.status_code}, " "not in the 200-300 range.",
        )
    ...
    def test_queue_return_multiple_outparam(self):
        r = self.webhost.request("POST", "put_queue_multiple_out", data="foo")
        self.assertTrue(
            200 <= r.status_code < 300,
            f"Returned status code {r.status_code}, " "not in the 200-300 range.",
        )
...
class TestQueueFunctionsStein(TestQueueFunctions):
    ...
    @classmethod
    def get_script_dir(cls):
        return testutils.E2E_TESTS_FOLDER / "queue_functions" / "queue_functions_stein"
