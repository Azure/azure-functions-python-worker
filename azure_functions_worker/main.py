...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="log level: 'TRACE', 'INFO', 'WARNING', " "or 'ERROR'",
    )
    parser.add_argument(
        "--log-to",
        type=str,
...
def main():
    ...
    try:
        return aio_compat.run(
            start_async(args.host, args.port, args.worker_id, args.request_id)
        )
    except Exception as ex:
        error_logger.exception(
            "unhandled error in functions worker: {0}".format(format_exception(ex))
        )
        raise
