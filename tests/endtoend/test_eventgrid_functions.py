...
class TestEventGridFunctions(testutils.WebHostTestCase):
    ...
    def eventgrid_webhook_request(self, meth, funcname, *args, **kwargs):
        ...
        return request_method(url, *args, params=params, headers=headers, **kwargs)

    @unittest.skip(
        "Run locally. Running on Azure fails with 401/403 as the"
    ...
    def test_eventgrid_trigger(self):
        ...
        r = self.eventgrid_webhook_request("POST", "eventgrid_trigger", json=data)
    ...
    def test_eventgrid_output_binding(self):
        ...
        expected_response = (
            "Sent event with subject: {}, id: {}, data: {}, "
        ...
        r = self.webhost.request(
            "GET", "eventgrid_output_binding", params={"test_uuid": test_uuid}
        )
        self.assertEqual(r.status_code, 200)
        response = r.text
        ...
        for try_no in range(max_retries):
            ...
            try:
                # Check that the trigger has fired.
                r = self.webhost.request("GET", "eventgrid_output_binding_success")
                self.assertEqual(r.status_code, 200)
                response = r.json()
