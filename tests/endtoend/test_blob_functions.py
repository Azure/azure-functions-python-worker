...
class TestBlobFunctionsStein(TestBlobFunctions):
    ...
    def test_blob_trigger_lacs_source(self):
        # lacs stands for LogsAndContainerScan, which is the standard polling
        # mechanism for BlobTrigger
        data = "DummyData"

        r = self.webhost.request('POST', 'put_blob_trigger_lacs_source',
                                 data=data.encode('utf-8'))
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.text, 'OK')
        ...
        for try_no in range(max_retries):
            ...
            try:
                # Check that the trigger has fired
                r = self.webhost.request('GET', 'get_blob_lacs_source_triggered')
                self.assertEqual(r.status_code, 200)
                response = r.json()

                self.assertEqual(
                    response['name'],
                    'python-worker-tests/test-blob-lacs-source-trigger.txt')
                self.assertEqual(response['content'], data)
    ...
    def test_blob_trigger_lacs_source_with_large_content(self):
        data = 'DummyDataDummyDataDummyData' * 1024 * 1024  # 27 MB
    ...
    def test_blob_trigger_eg_source(self):
        # eg stands for EventGrid, which is a lower latency polling
        # mechanism for BlobTrigger
        data = "DummyData"

        r = self.webhost.request('POST', 'put_blob_trigger_eg_source',
                                 data=data.encode('utf-8'))
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.text, 'OK')
        ...
        for try_no in range(max_retries):
            ...
            try:
                # Check that the trigger has fired
                r = self.webhost.request('GET', 'get_blob_eg_source_triggered')
                self.assertEqual(r.status_code, 200)
                response = r.json()

                self.assertEqual(response['name'],
                                 'python-worker-tests/test-blob-eg-source-trigger.txt')
                self.assertEqual(response['content'], data)
    ...
    def test_blob_trigger_eg_source_with_large_content(self):
        data = 'DummyDataDummyDataDummyData' * 1024 * 1024  # 27 MB
