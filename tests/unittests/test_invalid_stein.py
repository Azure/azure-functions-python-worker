...
STEIN_INVALID_APP_FUNCTIONS_DIR = (
    testutils.UNIT_TESTS_FOLDER / "broken_functions" / "invalid_app_stein"
)
STEIN_INVALID_FUNCTIONS_DIR = (
...
class TestInvalidStein(testutils.AsyncTestCase):
    ...
    def setUp(self):
        self._ctrl = testutils.start_mockhost(script_root=STEIN_INVALID_FUNCTIONS_DIR)
        self._pre_env = dict(os.environ)
        self.mock_version_info = patch(
            "azure_functions_worker.dispatcher.sys.version_info",
            SysVersionInfo(3, 9, 0, "final", 0),
