...
class TestGRPCandProtobufDependencyIsolationOnDedicated(testutils.WebHostTestCase):
    ...
    def test_paths_resolution(self):
        ...
        self.assertEqual(dm["cx_working_dir"].lower(), str(self.project_root).lower())
        self.assertEqual(dm["cx_deps_path"].lower(), str(self.customer_deps).lower())
    ...
    def test_loading_libraries_from_customers_package(self):
        ...
        self.assertEqual(libraries["grpc.expected.version"], libraries["grpc.version"])


@skip("Skipping dependency isolation test for dedicated. Needs investigation")
class TestOlderVersionOfAzFuncDependencyIsolationOnDedicated(testutils.WebHostTestCase):
    """Test the dependency manager E2E scenario via Http Trigger.
    ...
    def test_loading_libraries_from_customers_package(self):
        ...
        self.assertEqual(self.expected_azfunc_version, libraries["func.version"])


@skip("Skipping dependency isolation test for dedicated. Needs investigation")
class TestNewerVersionOfAzFuncDependencyIsolationOnDedicated(testutils.WebHostTestCase):
    """Test the dependency manager E2E scenario via Http Trigger.
