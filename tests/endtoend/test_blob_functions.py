...
class TestBlobFunctions(testutils.WebHostTestCase):
    ...
    def test_blob_io_bytes(self):
        r = self.webhost.request(
            "POST", "put_blob_bytes", data="test-dată".encode("utf-8")
        )
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.text, "OK")
    ...
    def test_blob_io_large_bytes(self):
        ...
        r = self.webhost.request(
            "POST", "put_blob_bytes", data=large_string.encode("utf-8")
        )
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.text, "OK")
    ...
    def test_blob_trigger(self):
        ...
        r = self.webhost.request("POST", "put_blob_trigger", data=data.encode("utf-8"))
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.text, "OK")
        ...
        for try_no in range(max_retries):
            ...
            try:
                ...
                self.assertEqual(
                    response["name"], "python-worker-tests/test-blob-trigger.txt"
                )
...
class TestBlobFunctionsStein(TestBlobFunctions):
    ...
    @classmethod
    def get_script_dir(cls):
        return testutils.E2E_TESTS_FOLDER / "blob_functions" / "blob_functions_stein"
