...
async def hijack_current_event_loop():
    ...
    return Response(content="OK-hijack-current-event-loop", media_type="text/plain")


@fast_app.get("/print_logging")
async def print_logging(
    message: str = "",
    flush: str = "false",
    console: str = "false",
    is_stderr: str = "false",
...
async def return_http_redirect(request: Request, code: str = ""):
    ...
    return RedirectResponse(
        status_code=302, url=f"http://{request.url.components[1]}/" f"{location}"
    )
