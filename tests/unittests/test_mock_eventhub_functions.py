...
class TestEventHubMockFunctions(testutils.AsyncTestCase):
    ...
    async def test_mock_eventhub_trigger_iot(self):
        async with testutils.start_mockhost(script_root=self.mock_funcs_dir) as host:
            ...
            async def call_and_check():
                _, r = await host.invoke_function(
                    ...
                    metadata={
                        "SystemProperties": protos.TypedData(
                            json=json.dumps(
                                {
                ...
                [
                    protos.ParameterBinding(
                        ...
                        data=protos.TypedData(
                            json=json.dumps({"id": "cardinality_one"})
                        ),
                    ),
                ],
                metadata={
    ...
    async def test_mock_eventhub_cardinality_one_bad_annotation(self):
        async with testutils.start_mockhost(script_root=self.mock_funcs_dir) as host:
            ...
            func_id, r = await host.load_function("eventhub_cardinality_one_bad_anno")
            self.assertEqual(r.response.function_id, func_id)
            ...
            _, r = await host.invoke_function(
                ...
                [
                    protos.ParameterBinding(
                        name="events",
                        data=protos.TypedData(
                            json=json.dumps([{"id": "cardinality_many"}])
                        ),
                    ),
                ],
                metadata={
                    "SystemPropertiesArray": protos.TypedData(
                        json=json.dumps(
                            [
                                {
                                    "iothub-device-id": "mock-iothub-device-id",
    ...
    async def test_mock_eventhub_cardinality_many_bad_annotation(self):
        async with testutils.start_mockhost(script_root=self.mock_funcs_dir) as host:
            ...
            func_id, r = await host.load_function("eventhub_cardinality_many_bad_anno")
            self.assertEqual(r.response.function_id, func_id)
            self.assertEqual(r.response.result.status, protos.StatusResult.Failure)
