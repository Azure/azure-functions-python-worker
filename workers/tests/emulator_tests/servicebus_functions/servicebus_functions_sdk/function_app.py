...
@app.route(route="put_message_sdk_deadletter")
@app.service_bus_queue_output(
    arg_name="msg",
    connection="AzureWebJobsServiceBusSDKConnectionString",
    queue_name="testqueue")
def put_message_sdk_deadletter(req: func.HttpRequest, msg: func.Out[str]):
    msg.set(req.get_body().decode('utf-8'))
    return 'OK'


@app.route(route="get_servicebus_triggered_sdk_deadletter")
@app.blob_input(arg_name="file",
                path="python-worker-tests/test-servicebus-sdk-triggered-deadletter.txt",
                connection="AzureWebJobsStorage")
def get_servicebus_triggered_sdk_deadletter(req: func.HttpRequest,
                                            file: func.InputStream) -> str:
    return func.HttpResponse(
        file.read().decode('utf-8'), mimetype='application/json')
