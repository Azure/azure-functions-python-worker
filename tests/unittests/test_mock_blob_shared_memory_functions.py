...
class TestMockBlobSharedMemoryFunctions(
    ...
    async def test_binary_blob_write_function(self):
        ...
        async with testutils.start_mockhost(
            script_root=self.blob_funcs_dir
        ) as host:
            ...
            mem_map = self.file_accessor.open_mem_map(
                mem_map_name, mem_map_size
            )
            shared_mem_map = SharedMemoryMap(
                self.file_accessor, mem_map_name, mem_map
            )
        ...
        async with testutils.start_mockhost(
            ...
            _, response_msg = await host.invoke_function(
                ...
                [
                    ...
                    protos.ParameterBinding(
                        name="file", rpc_shared_memory=value
                    ),
                ],
            )
    ...
    async def test_close_shared_memory_maps(self):
        ...
        async with testutils.start_mockhost(
            ...
            response_msg = await host.close_shared_memory_resources(
                mem_map_names
            )
