...
class SharedMemoryConstants:
    ...
    CONTENT_HEADER_TOTAL_BYTES = MEM_MAP_INITIALIZED_FLAG_NUM_BYTES + CONTENT_LENGTH_NUM_BYTES

    # A flag to indicate that the memory map has been initialized, may be in use and is not new.
    ...
    # Minimum size (in number of bytes) an object must be in order for it to be transferred over
    # shared memory.
    ...
    MIN_OBJECT_BYTES_FOR_SHARED_MEMORY_TRANSFER = 1024 * 1024 # 1 MB

    # Maximum size (in number of bytes) an object must be in order for it to be transferred over
    # shared memory.
    # This limit is imposed because initializing objects like greater than 2GB is not allowed in
    # DotNet.
    # Ref: https://stackoverflow.com/a/3944336/3132415
    # Note: This needs to be consistent among the host and workers.
    #       e.g. in the host, it is defined in SharedMemoryConstants.cs
    MAX_OBJECT_BYTES_FOR_SHARED_MEMORY_TRANSFER = 1024 * 1024 # 1 MB

    # This is what the size of a character is in DotNet. Can be verified by doing "sizeof(char)".
    # To keep the limits consistent, when determining if a string can be transferred over shared
    # memory, we multiply the number of characters by this constant.
