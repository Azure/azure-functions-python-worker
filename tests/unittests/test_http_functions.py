...
class TestHttpFunctions(testutils.WebHostTestCase):
    ...
    def test_return_http_auth_level_admin(self):
        r = self.webhost.request(
            "GET", "return_http_auth_admin", params={"code": "testMasterKey"}
        )
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.text, "<h1>Hello World™</h1>")
        self.assertEqual(r.headers["content-type"], "text/html; charset=utf-8")
    ...
    def test_return_http_redirect(self):
        ...
        r = self.webhost.request("GET", "return_http_redirect", allow_redirects=False)
    ...
    def test_accept_json(self):
        r = self.webhost.request("POST", "accept_json", json={"a": "abc", "d": 42})
    ...
    def check_log_unhandled_error(self, host_out: typing.List[str]):
        self.assertIn("Exception: ZeroDivisionError: division by zero", host_out)
    ...
    def test_unhandled_unserializable_error(self):
        r = self.webhost.request("GET", "unhandled_unserializable_error")
        self.assertEqual(r.status_code, 500)
    ...
    def check_log_import_module_troubleshooting_url(self, host_out: typing.List[str]):
        passed = False
        exception_message = (
    ...
    def test_print_logging_with_flush(self):
        r = self.webhost.request("GET", "print_logging?flush=true&message=Secret42")
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.text, "OK-print-logging")
    ...
    def test_print_to_console_stdout(self):
        r = self.webhost.request("GET", "print_logging?console=true&message=Secret42")
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.text, "OK-print-logging")

    @skipIf(sys.version_info < (3, 8, 0), "Skip the tests for Python 3.7 and below")
    ...
    def test_set_cookie_header_in_response_default_value(self):
        r = self.webhost.request("GET", "set_cookie_resp_header_default_values")
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.headers.get("Set-Cookie"), "foo=bar; domain=; path=")

    @skipIf(sys.version_info < (3, 8, 0), "Skip the tests for Python 3.7 and below")
    def test_response_cookie_header_nullable_timestamp_err(self):
        r = self.webhost.request("GET", "response_cookie_header_nullable_timestamp_err")
        self.assertEqual(r.status_code, 500)

    def check_log_response_cookie_header_nullable_timestamp_err(
        self, host_out: typing.List[str]
    ):
        self.assertIn(
    ...
    def test_response_cookie_header_nullable_bool_err(self):
        r = self.webhost.request("GET", "response_cookie_header_nullable_bool_err")
        self.assertEqual(r.status_code, 200)
        self.assertFalse("Set-Cookie" in r.headers)

    @skipIf(sys.version_info < (3, 8, 0), "Skip the tests for Python 3.7 and below")
    def test_response_cookie_header_nullable_double_err(self):
        r = self.webhost.request("GET", "response_cookie_header_nullable_double_err")
        self.assertEqual(r.status_code, 200)
        self.assertFalse("Set-Cookie" in r.headers)
    ...
    def test_print_to_console_stderr(self):
        r = self.webhost.request(
            "GET", "print_logging?console=true" "&message=Secret42&is_stderr=true"
        )
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.text, "OK-print-logging")

    def check_log_print_to_console_stderr(
        self,
...
class TestHttpFunctionsStein(TestHttpFunctions):
    ...
    @classmethod
    def get_script_dir(cls):
        return testutils.UNIT_TESTS_FOLDER / "http_functions" / "http_functions_stein"
