...
class TestExtension(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self._dummy_sdk_sys_path = os.path.join(
            os.path.dirname(__file__), "resources", "mock_azure_functions"
        )
        ...
        self._mock_context = MockContext(
            function_name=self._mock_func_name, function_directory=self._mock_func_dir
        )
    ...
    @patch(
        "azure_functions_worker.extension.ExtensionManager."
        "_warn_sdk_not_support_extension"
    )
    def test_function_load_extension_warns_when_sdk_does_not_support(
        self, _warn_sdk_not_support_extension_mock: Mock
    ):
    ...
    def test_function_load_extension_should_invoke_extension_call(
        self, safe_execute_function_load_hooks_mock: Mock
    ):
    ...
    def test_invocation_extension_should_invoke_extension_call(
        self, safe_execute_invocation_hooks_mock: Mock
    ):
        """Should invoke extension if SDK suports extension interface"""
        for hook_name in (
            APP_EXT_PRE_INVOCATION,
            ...
            self._instance._invocation_extension(
                ctx=self._mock_context, hook_name=hook_name, func_args=[], func_ret=None
            )

            safe_execute_invocation_hooks_mock.assert_has_calls(
                calls=[call(None, hook_name, self._mock_context, [], None)],
                any_order=True,
            )

    @patch(
        "azure_functions_worker.extension.ExtensionManager." "_is_pre_invocation_hook"
    )
    @patch(
        "azure_functions_worker.extension.ExtensionManager." "_is_post_invocation_hook"
    )
    def test_empty_hooks_should_not_receive_any_invocation(
        self, _is_post_invocation_hook_mock: Mock, _is_pre_invocation_hook_mock: Mock
    ):
    ...
    def test_invocation_hooks_should_be_executed(self):
        ...
        FuncExtClass = self._generate_new_func_extension_class(
            base=self._sdk.FuncExtensionBase, trigger=self._mock_func_name
        )
        func_ext_instance = FuncExtClass()
        hook_instances = self._sdk.ExtensionMeta.get_function_hooks(
            self._mock_func_name
        )
    ...
    def test_invocation_hooks_app_level_should_be_executed(self):
        ...
        safe_execute_invocation_hooks
        """
        AppExtClass = self._generate_new_app_extension(base=self._sdk.AppExtensionBase)
        hook_instances = self._sdk.ExtensionMeta.get_application_hooks()
    ...
    def test_exception_handling_in_post_function_load_app_level(self, error_mock: Mock):
        ...
        class BadAppExtension(self._sdk.AppExtensionBase):
            ...
            def post_function_load_app_level(
                cls, function_name, function_directory, *args, **kwargs
            ):
    ...
    def test_exception_handling_in_pre_invocation_app_level(self):
        ...
        class BadAppExtension(self._sdk.AppExtensionBase):
            ...
            def pre_invocation_app_level(
                cls, logger, context, func_args, *args, **kwargs
            ):
                raise expt

        # Create a mocked customer_function
        wrapped = self._instance.get_sync_invocation_wrapper(
            self._mock_context, self._mock_function_main
        )
    ...
    def test_get_async_invocation_wrapper_no_extension(self):
        ...
        result = aio_compat.run(
            self._instance.get_async_invocation_wrapper(
                self._mock_context, self._mock_function_main_async, self._mock_arguments
            )
        )
    ...
    def test_is_pre_invocation_hook(self):
        for name in (FUNC_EXT_PRE_INVOCATION, APP_EXT_PRE_INVOCATION):
            self.assertTrue(self._instance._is_pre_invocation_hook(name))
    ...
    def test_is_pre_invocation_hook_negative(self):
        for name in (
            ...
            self.assertFalse(self._instance._is_pre_invocation_hook(name))

    def test_is_post_invocation_hook(self):
        for name in (FUNC_EXT_POST_INVOCATION, APP_EXT_POST_INVOCATION):
            self.assertTrue(self._instance._is_post_invocation_hook(name))
    ...
    def test_is_post_invocation_hook_negative(self):
        for name in (
            ...
            self.assertFalse(self._instance._is_post_invocation_hook(name))

    @patch(
        "azure_functions_worker.extension."
        "ExtensionManager._info_extension_is_enabled"
    )
    def test_try_get_sdk_with_extension_enabled_should_execute_once(
        self, info_extension_is_enabled_mock: Mock
    ):
    ...
    def test_try_get_sdk_with_extension_disabled_should_execute_once(
        self, warn_sdk_not_support_extension_mock: Mock
    ):
    ...
    def test_info_discover_extension_list_func_ext(self, info_mock: Mock):
        ...
        FuncExtClass = self._generate_new_func_extension_class(
            sdk.FuncExtensionBase, self._mock_func_name
        )
        ...
        info_mock.assert_called_once_with(
            ...
            "current registered extensions: %s",
            "HttpTrigger",
    ...
    def _generate_new_func_extension_class(self, base: type, trigger: str):
        class NewFuncExtension(base):
            ...
            def post_function_load(
                self, function_name, function_directory, *args, **kwargs
            ):
