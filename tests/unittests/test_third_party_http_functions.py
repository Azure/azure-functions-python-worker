...
class ThirdPartyHttpFunctionsTestBase:
    class TestThirdPartyHttpFunctions(testutils.WebHostTestCase):
        ...
        def test_debug_with_user_logging(self):
            r = self.webhost.request("GET", "debug_user_logging", no_prefix=True)
            self.assertEqual(r.status_code, 200)
        ...
        def test_print_logging_no_flush(self):
            r = self.webhost.request(
                "GET", "print_logging?message=Secret42", no_prefix=True
            )
            self.assertEqual(r.status_code, 200)
        ...
        def test_print_logging_with_flush(self):
            r = self.webhost.request(
                "GET", "print_logging?flush=true&message" "=Secret42", no_prefix=True
            )
            self.assertEqual(r.status_code, 200)
            self.assertEqual(r.text, "OK-print-logging")
        ...
        def test_print_to_console_stdout(self):
            r = self.webhost.request(
                "GET", "print_logging?console=true&message" "=Secret42", no_prefix=True
            )
            self.assertEqual(r.status_code, 200)
            self.assertEqual(r.text, "OK-print-logging")
        ...
        def test_print_to_console_stderr(self):
            r = self.webhost.request(
                "GET",
                "print_logging?console=true" "&message=Secret42&is_stderr=true",
                no_prefix=True,
            )
            self.assertEqual(r.status_code, 200)
            self.assertEqual(r.text, "OK-print-logging")
        ...
        def test_raw_body_bytes(self):
            ...
            with open(image_file, "rb") as image:
                ...
                img_len = len(img)
                r = self.webhost.request(
                    "POST", "raw_body_bytes", data=img, no_prefix=True
                )
        ...
        def test_return_http_no_body(self):
            r = self.webhost.request("GET", "return_http_no_body", no_prefix=True)
            self.assertEqual(r.text, "")
            self.assertEqual(r.status_code, 200)

        def test_return_http_redirect(self):
            r = self.webhost.request("GET", "return_http_redirect", no_prefix=True)
            self.assertEqual(r.status_code, 200)
            self.assertEqual(r.text, "<h1>Hello World™</h1>")

            r = self.webhost.request(
                "GET", "return_http_redirect", allow_redirects=False, no_prefix=True
            )
...
class TestAsgiHttpFunctions(
    ...
    def get_script_dir(cls):
        return (
            UNIT_TESTS_ROOT / "third_party_http_functions" / "stein" / "asgi_function"
        )

    def test_hijack_current_event_loop(self):
        r = self.webhost.request("GET", "hijack_current_event_loop", no_prefix=True)
        self.assertEqual(r.status_code, 200)
...
class TestWsgiHttpFunctions(
    ...
    def get_script_dir(cls):
        return (
            UNIT_TESTS_ROOT / "third_party_http_functions" / "stein" / "wsgi_function"
        )
