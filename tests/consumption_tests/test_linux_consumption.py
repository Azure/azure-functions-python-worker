...
class TestLinuxConsumption(TestCase):
    ...
    def test_placeholder_mode_root_returns_ok(self):
        ...
        """
        with LinuxConsumptionWebHostController(
    ...
    @skipIf(
        sys.version_info.minor in (10, 11), "Protobuf pinning fails during remote build"
    )
    ...
    def test_pinning_functions_to_older_version(self):
        ...
        with LinuxConsumptionWebHostController(
            ...
            ctrl.assign_container(
                env={
                    "AzureWebJobsStorage": self._storage,
                    "SCM_RUN_FROM_PACKAGE": self._get_blob_url("PinningFunctions"),
                    "PYTHON_ISOLATE_WORKER_DEPENDENCIES": "1",
                }
            )
            req = Request("GET", f"{ctrl.url}/api/HttpTrigger1")
    ...
    @skipIf(sys.version_info.minor != 10, "This is testing only for python310")
    ...
    @skipIf(
        ...
        "This is testing only for python39 where extensions" "enabled by default",
    )
    def test_reload_variables_after_timeout_error(self):
        ...
        with LinuxConsumptionWebHostController(
            ...
            ctrl.assign_container(
                env={
                    "AzureWebJobsStorage": self._storage,
                    "SCM_RUN_FROM_PACKAGE": self._get_blob_url("TimeoutError"),
                    "PYTHON_ISOLATE_WORKER_DEPENDENCIES": "1",
                    "AzureWebJobsFeatureFlags": "EnableWorkerIndexing",
                }
            )
            ...
            self.assertNotIn("Failure Exception: ModuleNotFoundError", logs)
    ...
    def test_reload_variables_after_oom_error(self):
        ...
        with LinuxConsumptionWebHostController(
            ...
            ctrl.assign_container(
                env={
                    "AzureWebJobsStorage": self._storage,
                    "SCM_RUN_FROM_PACKAGE": self._get_blob_url("OOMError"),
                    "PYTHON_ISOLATE_WORKER_DEPENDENCIES": "1",
                    "AzureWebJobsFeatureFlags": "EnableWorkerIndexing",
                }
            )
