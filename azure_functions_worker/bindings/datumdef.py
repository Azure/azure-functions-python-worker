...
class Datum:
    ...
    def from_rpc_shared_memory(
        ...
        if ret_val is not None:
            logger.info(
                "Read %s bytes from memory map %s for data type %s",
                count,
                mem_map_name,
                data_type,
            )
    ...
    def to_rpc_shared_memory(
        ...
        if shared_mem_meta is None:
            logger.warning("Cannot write to shared memory for type: %s", datum.type)
            return None
        ...
        logger.info(
            ...
            shared_mem_meta.count_bytes,
            shared_mem_meta.mem_map_name,
            data_type,
        )
        return shmem
    ...
    for cookie in cookies:
        for name, cookie_entity in cookie.items():
            rpc_http_cookies.append(
                protos.RpcHttpCookie(
                    name=name,
                    value=cookie_entity.value,
                    domain=to_nullable_string(cookie_entity["domain"], "cookie.domain"),
                    path=to_nullable_string(cookie_entity["path"], "cookie.path"),
                    expires=to_nullable_timestamp(
                        parse_cookie_attr_expires(cookie_entity), "cookie.expires"
                    ),
                    secure=to_nullable_bool(
                        bool(cookie_entity["secure"]), "cookie.secure"
                    ),
                    http_only=to_nullable_bool(
                        bool(cookie_entity["httponly"]), "cookie.httpOnly"
                    ),
                    same_site=parse_cookie_attr_same_site(cookie_entity),
                    max_age=to_nullable_double(
                        cookie_entity["max-age"], "cookie.maxAge"
                    ),
                )
            )
