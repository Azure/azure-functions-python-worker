...
class TestHttpFunctions(testutils.WebHostTestCase):
    ...
    def test_return_http_redirect(self):
        ...
        r = self.webhost.request(
            "GET", "return_http_redirect", allow_redirects=False
        )
    ...
    def test_accept_json(self):
        r = self.webhost.request(
            "POST", "accept_json", json={"a": "abc", "d": 42}
        )
    ...
    def check_log_unhandled_error(self, host_out: typing.List[str]):
        self.assertIn(
            "Exception: ZeroDivisionError: division by zero", host_out
        )
    ...
    def check_log_import_module_troubleshooting_url(
    ...
    def test_print_logging_with_flush(self):
        r = self.webhost.request(
            "GET", "print_logging?flush=true&message=Secret42"
        )
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.text, "OK-print-logging")
    ...
    def test_print_to_console_stdout(self):
        r = self.webhost.request(
            "GET", "print_logging?console=true&message=Secret42"
        )
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.text, "OK-print-logging")
    ...
    def check_log_retry_context_exponential_backoff(
        self, host_out: typing.List[str]
    ):
        self.assertIn("Current retry count: 1", host_out)
        self.assertIn("Current retry count: 2", host_out)
