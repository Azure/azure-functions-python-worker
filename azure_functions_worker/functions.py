...
class FunctionLoadError(RuntimeError):
    ...
    def __init__(self, function_name: str, msg: str) -> None:
        super().__init__(f"cannot load the {function_name} function: {msg}")
...
class Registry:
    ...
    @staticmethod
    def get_explicit_and_implicit_return(
        binding_name: str,
        binding: BindingInfo,
    ...
    @staticmethod
    def get_return_binding(
        binding_name: str, binding_type: str, return_binding_name: str
    ) -> str:
        if binding_name == "$return":
    ...
    @staticmethod
    def validate_binding_direction(
        binding_name: str, binding_direction: str, func_name: str
    ):
        if binding_direction == protos.BindingInfo.inout:
            raise FunctionLoadError(func_name, '"inout" bindings are not supported')

        if binding_name == "$return" and binding_direction != protos.BindingInfo.out:
    ...
    def is_context_required(
        params, bound_params: dict, annotations: dict, func_name: str
    ) -> bool:
    ...
    def validate_function_params(
        params: dict, bound_params: dict, annotations: dict, func_name: str
    ):
        ...
        for param in params.values():
            ...
            if (
                param_has_anno
                and not isinstance(param_py_type, type)
            ...
            if (
                param_has_anno
                and param_py_type in (str, bytes)
                and (not bindings_utils.has_implicit_output(binding.type))
            ):
    ...
    def get_function_return_type(
        ...
        if has_explicit_return and "return" in annotations:
            ...
            if (
                typing_inspect.is_generic_type(return_anno)
    ...
    def add_function(
        ...
        for binding_name, binding_info in metadata.bindings.items():
            self.validate_binding_direction(
                binding_name, binding_info.direction, func_name
            )
            ...
            return_binding_name = self.get_return_binding(
                binding_name, binding_info.type, return_binding_name
            )
        ...
        input_types, output_types = self.validate_function_params(
            params, bound_params, annotations, func_name
        )

        return_type = self.get_function_return_type(
            annotations,
            has_explicit_return,
            has_implicit_return,
            return_binding_name,
    ...
    def add_indexed_function(self, function):
        ...
        for binding in function.get_bindings():
            self.validate_binding_direction(binding.name, binding.direction, func_name)
            ...
            return_binding_name = self.get_return_binding(
                binding.name, binding.type, return_binding_name
            )
        ...
        return self.add_func_to_registry_and_return_funcinfo(
            func,
            func_name,
            function_id,
            func_dir,
