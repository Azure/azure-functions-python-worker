...
class Dispatcher(metaclass=DispatcherMeta):
    ...
    def initialize_azure_monitor(self):
        ...
        try:
            ...
            logger.exception("Cannot import Azure Monitor distro.")
            self._azure_monitor_available = False
        except Exception:
            logger.exception("Error initializing Azure monitor distro.")
            self._azure_monitor_available = False
    ...
    def update_opentelemetry_status(self):
        ...
        try:
            ...
            logger.exception("Cannot import OpenTelemetry libraries.")
    ...
    async def _handle__functions_metadata_request(self, request):
        ...
        if self._function_metadata_exception:
            ...
            return protos.StreamingMessage(
                request_id=request.request_id,
                function_metadata_response=protos.FunctionMetadataResponse(
                    use_default_metadata_indexing=False if metadata_result else True,
        ...
        try:
            if not self._functions.get_function(function_id):
                ...
                if function_metadata.properties.get(
                    ...
                    try:
                        self.load_function_metadata(
                            function_app_directory, caller_info="functions_load_request"
                        )
                    except Exception as ex:
                        self._function_metadata_exception = ex
    ...
    async def _handle__invocation_request(self, request):
        ...
        try:
            ...
            http_v2_enabled = (
                self._functions.get_function(function_id).is_http_func
                and HttpV2Registry.http_v2_enabled()
            )
    ...
    def index_functions(self, function_path: str, function_dir: str):
        ...
        logger.info(
            "Indexed function app and found %s functions", len(indexed_functions)
        )

        if indexed_functions:
            fx_metadata_results, fx_bindings_logs = loader.process_indexed_function(
                self._functions, indexed_functions, function_dir
            )
            ...
            for func in indexed_functions:
                ...
                for binding in func.get_bindings():
                    deferred_binding_info = (
                        func_binding_logs.get(binding.name)
                        if func_binding_logs.get(binding.name)
                        else ""
                    )
            ...
            logger.info(
                ...
                "functions: %s. Deferred bindings enabled: %s.",
        ...
        try:
            for map_name in map_names:
                try:
                    ...
                    logger.error(
                        "Cannot free memory map %s - %s", map_name, e, exc_info=True
                    )
        finally:
    ...
    def configure_opentelemetry(self, invocation_context):
        carrier = {
            _TRACEPARENT: invocation_context.trace_context.trace_parent,
