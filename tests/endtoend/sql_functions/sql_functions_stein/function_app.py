...
@app.sql_input(arg_name="products_snake",
...
def sql_input(req: func.HttpRequest, products_snake: func.SqlRowList) \
        -> func.HttpResponse:
...
@app.sql_trigger(arg_name="changes_snake",
                 table_name="Products",
                 connection_string_setting="AzureWebJobsSqlConnectionString")
@app.sql_output(arg_name="r",
                command_text="[dbo].[Products2]",
                connection_string_setting="AzureWebJobsSqlConnectionString")
def sql_trigger(changes_snake, r: func.Out[func.SqlRow]) -> str:
    row = func.SqlRow.from_dict(json.loads(changes_snake)[0]["Item"])
    r.set(row)
    return "OK"
