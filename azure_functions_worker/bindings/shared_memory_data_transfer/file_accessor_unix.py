...
class FileAccessorUnix(FileAccessor):
    ...
    def delete_mem_map(self, mem_map_name: str, mem_map: mmap.mmap) -> bool:
        ...
        try:
            ...
            logger.error(
                "Cannot delete memory map %s - %s", mem_map_name, e, exc_info=True
            )
            return False
        mem_map.close()
        return True
    ...
    def _get_valid_mem_map_dirs(self) -> List[str]:
        ...
        for temp_dir in allowed_dirs:
            ...
            if os.path.exists(dir_path):
                ...
                logger.debug("Found directory %s to store memory maps", dir_path)
            else:
                try:
                    ...
                    logger.warning(
                        "Cannot create directory %s to " "store memory maps - %s",
                        dir_path,
                        e,
                        exc_info=True,
                    )
        if len(valid_dirs) == 0:
            logger.error("No valid directory for memory maps in %s", allowed_dirs)
        return valid_dirs
        ...
        for temp_dir in self.valid_dirs:
            ...
            if os.path.exists(file_path):
                try:
                    ...
                    logger.error(
                        "Cannot open file %s - %s", file_path, e, exc_info=True
                    )
        # The memory map was not found in any of the known directories
        logger.error(
            "Cannot open memory map %s in any of the following directories: " "%s",
            mem_map_name,
            self.valid_dirs,
        )
        return None
    ...
    def _create_mem_map_file(
        ...
        for temp_dir in self.valid_dirs:
            ...
            try:
                ...
                logger.warning(
                    "Cannot create memory map in %s - %s." " Trying other directories.",
                    file_path,
                    e,
                    exc_info=True,
                )
