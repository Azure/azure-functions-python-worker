...
class HttpCoordinator:
    ...
    def get_func_info_update_event(self, invoc_id):
        if invoc_id not in self._func_info_update_event:
            self._func_info_update_event[invoc_id] = asyncio.Event()
        return self._func_info_update_event[invoc_id]

    async def set_function_info(self, invoc_id, func, args):
        self._func_info[invoc_id] = (func, args)
        logger.info("-----> set_function_info %s", self._func_info)
        self.get_func_info_update_event(invoc_id).set()

    async def get_function_info(self, invoc_id):
        event = self.get_func_info_update_event(invoc_id)
        await event.wait()
        return self._func_info[invoc_id]
...
async def handle_request_get(request):
    ...
    loop = http_coordinator.get_loop()
    ...
    asyncio.run_coroutine_threadsafe(
        http_coordinator.add_http_invoc_request(invoc_id, request), loop
    ).result()

    # response = asyncio.run_coroutine_threadsafe(
    #     http_coordinator.wait_and_get_http_invoc_response(invoc_id), loop
    # ).result()

    func, args = asyncio.run_coroutine_threadsafe(
        http_coordinator.get_function_info(invoc_id), loop
    ).result()

    result = await func(**args)
    logger.info("server response completed  %s", result)
    return result
    ...
    # logger.info("server response completed")
    # return response
    # return web.Response(
...
class Dispatcher(metaclass=DispatcherMeta):
    ...
    def __init__(
        ...
        self._http_coordinator.set_loop(loop)

    @staticmethod
    def get_worker_metadata():
        return protos.WorkerMetadata(
            runtime_name=PYTHON_LANGUAGE_RUNTIME,
            runtime_version=f"{sys.version_info.major}." f"{sys.version_info.minor}",
        ...
        try:
            ...
            self._grpc_resp_queue.put_nowait(
                protos.StreamingMessage(
                    ...
                    start_stream=protos.StartStream(worker_id=self.worker_id),
                )
            )
    ...
    def on_logging(self, record: logging.LogRecord, formatted_msg: str) -> None:
        ...
        self._grpc_resp_queue.put_nowait(
            protos.StreamingMessage(
                request_id=self.request_id, rpc_log=protos.RpcLog(**log)
            )
        )
    ...
    async def _dispatch_grpc_request(self, request):
        ...
        logger.info("%s resp %s", time.time(), resp)
    ...
    async def _handle__functions_metadata_request(self, request):
        ...
        if not os.path.exists(function_path):
            ...
            logger.info(
                "%s does not exist. " "Switching to host indexing.",
                SCRIPT_FILE_NAME
            )
            return protos.StreamingMessage(
                request_id=request.request_id,
    ...
    async def _handle__invocation_request(self, request):
        ...
        try:
            ...
            if fi.is_async:
                await self._http_coordinator.set_function_info(
                    invocation_id, fi.func, args
                )
    ...
    async def _handle__function_environment_reload_request(self, request):
        ...
        try:
            ...
            if getattr(func_env_reload_request, "function_app_directory", None):
    ...
    def index_functions(self, function_path: str):
        ...
        if indexed_functions:
            ...
            logger.info(
                "Successfully processed FunctionMetadataRequest for " "functions: %s",
        ...
        try:
            for map_name in map_names:
                try:
                    ...
                    success = self._shmem_mgr.free_mem_map(
                        map_name, to_delete_resources
                    )
                    results[map_name] = success
                except Exception as e:
                    logger.error(
                        "Cannot free memory map %s - %s", map_name, e,
                        exc_info=True
                    )
        finally:
    ...
    def _create_sync_call_tp(
        ...
        running.
        """
        return concurrent.futures.ThreadPoolExecutor(max_workers=max_worker)
    ...
    def __poll_grpc(self):
        ...
        try:
            ...
            self._loop.call_soon_threadsafe(self._grpc_connected_fut.set_result,
                                            True)
...
class AsyncLoggingHandler(logging.Handler):
    def emit(self, record: LogRecord) -> None:
        ...
        try:
            ...
            print(
                f"{CONSOLE_LOG_PREFIX} ERROR: {str(runtime_error)}",
                file=sys.stderr,
                flush=True,
            )
...
class ContextEnabledTask(asyncio.Task):
    ...
    def __init__(self, coro, loop):
        ...
        if current_task is not None:
            invocation_id = getattr(current_task, self.AZURE_INVOCATION_ID,
                                    None)
            if invocation_id is not None:
    ...
    if loop is not None:
        ...
        if current_task is not None:
            task_invocation_id = getattr(
                current_task, ContextEnabledTask.AZURE_INVOCATION_ID, None
            )
