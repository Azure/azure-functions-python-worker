    ...
    async def _handle__worker_init_request(self, request):
        ...
        if os.path.exists(v2_directory):
            ...
            try:
                ...
                logger.info(f"Error when importing V1 library: {traceback.format_exc()}")
    ...
    async def _handle__function_environment_reload_request(self, request):
        ...
        if os.path.exists(v2_directory):
            try:
                ...
                logger.info(
                    f"Error when importing V2 library: {traceback.format_exc()}")
        else:
            try:
                logger.info("Trying to import v1 worker")
                import azure_functions_worker_v1
                ...
                logger.info(
                    f"Error when importing V1 library: {traceback.format_exc()}")
