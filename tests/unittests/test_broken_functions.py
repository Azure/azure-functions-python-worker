...
class TestMockHost(testutils.AsyncTestCase):
    ...
    async def test_load_broken__missing_py_param(self):
        async with testutils.start_mockhost(script_root=self.broken_funcs_dir) as host:
    ...
    async def test_load_broken__module_not_found_error(self):
        async with testutils.start_mockhost(script_root=self.broken_funcs_dir) as host:
            ...
            self.assertIn("ModuleNotFoundError", r.response.result.exception.message)
    ...
    async def test_load_broken__import_error(self):
        async with testutils.start_mockhost(script_root=self.broken_funcs_dir) as host:
            ...
            self.assertIn("ImportError", r.response.result.exception.message)
            self.assertNotIn(
                "<frozen importlib._bootstrap>", r.response.result.exception.message
            )
            self.assertNotIn(
    ...
    async def test_load_broken__inout_param(self):
        async with testutils.start_mockhost(script_root=self.broken_funcs_dir) as host:
            ...
            self.assertRegex(
                r.response.result.exception.message,
                r".*cannot load the inout_param function" r'.*"inout" bindings.*',
            )
    ...
    async def test_load_broken__invalid_return_anno_non_type(self):
        async with testutils.start_mockhost(script_root=self.broken_funcs_dir) as host:
            func_id, r = await host.load_function("invalid_return_anno_non_type")
    ...
    async def test_import_module_troubleshooting_url(self):
        async with testutils.start_mockhost(script_root=self.broken_funcs_dir) as host:
            ...
            self.assertRegex(
                r.response.result.exception.message, r".*ModuleNotFoundError"
            )
