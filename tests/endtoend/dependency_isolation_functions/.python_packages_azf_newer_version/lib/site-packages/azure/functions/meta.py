...
from ._utils import try_parse_datetime_with_formats, try_parse_timedelta_with_formats


def is_iterable_type_annotation(annotation: object, pytype: object) -> bool:
    is_iterable_anno = typing_inspect.is_generic_type(annotation) and issubclass(
        typing_inspect.get_origin(annotation), collections.abc.Iterable
    )
    ...
    if isinstance(pytype, tuple):
        return any(
            isinstance(t, type) and issubclass(t, arg) for t in pytype for arg in args
        )
    else:
        return any(isinstance(pytype, type) and issubclass(pytype, arg) for arg in args)
...
class _BaseConverter(metaclass=_ConverterMeta, binding=None):
    ...
    def _parse_datetime_metadata(
        cls, trigger_metadata: Mapping[str, Datum], field: str
    ...
    def _parse_datetime(
        ...
        if too_fractional:
            ...
            datetime_str = too_fractional.group(1) + (too_fractional.group(3) or "")
    ...
    def _parse_timedelta(
        cls, timedelta_str: Optional[str]
    ) -> Optional[datetime.timedelta]:
    ...
    def _parse_datetime_utc(
        ...
        dt, _, excpt = try_parse_datetime_with_formats(datetime_str, utc_formats)
    ...
    def _parse_datetime_local(
        ...
        dt, _, excpt = try_parse_datetime_with_formats(datetime_str, local_formats)
