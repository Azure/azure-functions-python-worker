...
class TestFunctions(unittest.TestCase):
    ...
    def test_http_response(self):
        r = azf.HttpResponse(
            "body™", status_code=201, headers=dict(aaa="zzz", bAb="xYz")
        )
...
class TestTriggerMetadataDecoder(unittest.TestCase):
    ...
    def test_scalar_typed_data_decoder_not_ok(self):
        metadata = {
            "unsupported_type": bind_meta.Datum(type="bytes", value=b"aaa"),
            "unexpected_json": bind_meta.Datum(type="json", value="[1, 2, 3]"),
            "unexpected_data": bind_meta.Datum(type="json", value='"foo"'),
        }
