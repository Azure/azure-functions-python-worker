...
class IsUtilityTestCase(TestCase):
    ...
    def test_callable(self):
        samples = [Callable, Callable[..., int], Callable[[int, int], Iterable[str]]]
        nonsamples = [int, type, 42, [], List[int], Union[callable, Callable[..., int]]]
...
class GetUtilityTestCase(TestCase):
    ...
    def test_origin(self):
        ...
        self.assertEqual(
            get_origin(List[Tuple[T, T]][int]), list if NEW_TYPING else List
        )
    ...
    def test_args(self):
        ...
        self.assertEqual(get_args(Union[int, Union[T, int], str][int]), (int, str))
    ...
    def test_args_evaluated(self):
        ...
        self.assertEqual(
            get_args(Callable[[], T][int], evaluate=True),
            (
                [],
                int,
            ),
