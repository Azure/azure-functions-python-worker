...
class TestThreadPoolSettingsPython37(testutils.AsyncTestCase):
    ...
    def setUp(self, version=SysVersionInfo(3, 7, 0, "final", 0)):
        self._ctrl = testutils.start_mockhost(script_root=DISPATCHER_FUNCTIONS_DIR)
        self._default_workers: Optional[int] = PYTHON_THREADPOOL_THREAD_COUNT_DEFAULT
        ...
        self.mock_version_info = patch(
            "azure_functions_worker.dispatcher.sys.version_info", version
        )
    ...
    async def test_dispatcher_initialize_worker(self):
        ...
        async with self._ctrl as host:
            ...
            self.assertIsInstance(r.response.worker_metadata, protos.WorkerMetadata)
            self.assertEquals(r.response.worker_metadata.runtime_name, "python")
            self.assertEquals(r.response.worker_metadata.worker_version, VERSION)
    ...
    async def test_dispatcher_environment_reload(self):
        ...
        async with self._ctrl as host:
            ...
            self.assertIsInstance(r.response, protos.FunctionEnvironmentReloadResponse)
            self.assertIsInstance(r.response.worker_metadata, protos.WorkerMetadata)
            self.assertEquals(r.response.worker_metadata.runtime_name, "python")
            self.assertEquals(r.response.worker_metadata.worker_version, VERSION)
            ...
            self.assertEqual(
                len(
                    [
                        log
                        for log in r.logs
                        if log.message.startswith("Received WorkerInitRequest")
                    ]
                ),
                1,
            )
    ...
    async def test_dispatcher_sync_threadpool_default_worker(self):
        ...
        async with self._ctrl as host:
            # await self._check_if_function_is_ok(host)
            await self._assert_workers_threadpool(
                self._ctrl, host, self._default_workers
            )
    ...
    async def test_dispatcher_sync_threadpool_set_worker(self):
        ...
        async with self._ctrl as host:
            await self._check_if_function_is_ok(host)
            await self._assert_workers_threadpool(
                self._ctrl, host, self._allowed_max_workers
            )
    ...
    async def test_dispatcher_sync_threadpool_in_placeholder_invalid(self):
        ...
        with patch("azure_functions_worker.dispatcher.logger") as mock_logger:
            async with self._ctrl as host:
                ...
                # Reload environment variable on specialization
                await host.reload_environment(
                    environment={PYTHON_THREADPOOL_THREAD_COUNT: "invalid"}
                )
                await self._assert_workers_threadpool(
                    self._ctrl, host, self._default_workers
                )
    ...
    async def test_dispatcher_sync_threadpool_in_placeholder_below_min(self):
        ...
        with patch("azure_functions_worker.dispatcher.logger") as mock_logger:
            async with self._ctrl as host:
                ...
                # Reload environment variable on specialization
                await host.reload_environment(
                    environment={PYTHON_THREADPOOL_THREAD_COUNT: "0"}
                )
    ...
    async def test_sync_invocation_request_log(self):
        with patch("azure_functions_worker.dispatcher.logger") as mock_logger:
            async with self._ctrl as host:
                ...
                func_id, invoke_id, func_name = await self._check_if_function_is_ok(
                    host
                )
    ...
    async def test_sync_invocation_request_log_in_placeholder_threads(self):
        with patch("azure_functions_worker.dispatcher.logger") as mock_logger:
            async with self._ctrl as host:
                await host.reload_environment(
                    environment={PYTHON_THREADPOOL_THREAD_COUNT: "5"}
                )
    ...
    async def _assert_workers_threadpool(self, ctrl, host, expected_worker_count):
        ...
        self.assertEqual(ctrl._worker.get_sync_tp_workers_set(), expected_worker_count)
    ...
    async def _check_if_function_is_ok(self, host) -> Tuple[str, str, str]:
        ...
        self.assertEqual(
            load_r.response.result.status, protos.StatusResult.Success, msg=ex
        )
        ...
        invoke_id, call_r = await host.invoke_function(
            ...
            [
                protos.ParameterBinding(
                    name="req", data=protos.TypedData(http=protos.RpcHttp(method="GET"))
                )
            ],
        )
        self.assertIsNotNone(invoke_id)
        self.assertEqual(call_r.response.result.status, protos.StatusResult.Success)
    ...
    async def _check_if_async_function_is_ok(self, host) -> Tuple[str, str]:
        ...
        self.assertEqual(load_r.response.function_id, func_id)
        self.assertEqual(load_r.response.result.status, protos.StatusResult.Success)
    ...
    async def test_dispatcher_sync_threadpool_in_placeholder_above_max(self):
        ...
        with patch("azure_functions_worker.dispatcher.logger"):
            async with self._ctrl as host:
                ...
                self.assertNotEqual(
                    self._ctrl._worker.get_sync_tp_workers_set(), self._default_workers
                )
...
class TestThreadPoolSettingsPython39(TestThreadPoolSettingsPython38):
    def setUp(self, version=SysVersionInfo(3, 9, 0, "final", 0)):
        ...
        self.mock_os_cpu = patch("os.cpu_count", return_value=2)
