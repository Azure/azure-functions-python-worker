...
@app.route(route="get_servicebus_triggered_topic")
@app.blob_input(arg_name="file",
                path="python-worker-tests/test-servicebus-triggered-topic.txt",
                connection="AzureWebJobsStorage")
def get_servicebus_triggered_topic(req: func.HttpRequest,
                             file: func.InputStream) -> str:
    return func.HttpResponse(
        file.read().decode('utf-8'), mimetype='application/json')
...
@app.blob_output(arg_name="$return",
                 path="python-worker-tests/test-servicebus-triggered-topic.txt",
                 connection="AzureWebJobsStorage")
def servicebus_trigger_topic(msg: func.ServiceBusMessage) -> str:
    result = json.dumps({
        'message_id': msg.message_id,
        'body': msg.get_body().decode('utf-8'),
        'content_type': msg.content_type,
