...
class SharedMemoryManager:
    ...
    def put_bytes(self, content: bytes) -> Optional[SharedMemoryMetadata]:
        ...
        try:
            ...
            logger.warning(
                "Cannot write %s bytes into shared memory %s - %s",
                content_length,
                mem_map_name,
                e,
            )
    ...
    def get_bytes(self, mem_map_name: str, offset: int, count: int) -> Optional[bytes]:
        ...
        if offset != 0:
            logger.error(
                "Cannot read bytes. Non-zero offset (%s) not supported.", offset
            )
            return None
    ...
    def free_mem_map(
        ...
        if mem_map_name not in self.allocated_mem_maps:
            logger.error(
                "Cannot find memory map in list of allocations %s", mem_map_name
            )
            return False
