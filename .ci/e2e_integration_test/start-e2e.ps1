...
coverage run -m pytest --junitxml=e2e-integration-test-report.xml --reruns 4 --cov=./azure_functions_worker --cov-branch --cov-append tests/endtoend --cov-report xml
Write-Host "-----------------------------------------------------------------------------`n" -ForegroundColor Green
