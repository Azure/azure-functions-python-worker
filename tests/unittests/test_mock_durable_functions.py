...
class TestDurableFunctions(testutils.AsyncTestCase):
    ...
    async def test_mock_activity_trigger(self):
        async with testutils.start_mockhost(
            script_root=self.durable_functions_dir
        ) as host:
            ...
            _, r = await host.invoke_function(
                ...
                [
                    ...
                    protos.ParameterBinding(
                        name="input", data=protos.TypedData(string="test single_word")
                    )
                ],
            )
            self.assertEqual(r.response.result.status, protos.StatusResult.Success)
            self.assertEqual(
                r.response.return_value, protos.TypedData(json='"test single_word"')
            )
        ...
        async with testutils.start_mockhost(
            ...
            _, r = await host.invoke_function(
                ...
                [
                    ...
                    protos.ParameterBinding(
                        name="input",
                        data=protos.TypedData(string="test multiple words"),
                    )
                ],
            )
            self.assertEqual(r.response.result.status, protos.StatusResult.Success)
            self.assertEqual(
                r.response.return_value, protos.TypedData(json='"test multiple words"')
            )
        ...
        async with testutils.start_mockhost(
            ...
            _, r = await host.invoke_function(
                ...
                [
                    ...
                    # Activity Trigger's input must be json serializable
                    protos.ParameterBinding(
                        name="input", data=protos.TypedData(json='{"bird": "Crane"}')
                    )
                ],
            )
            self.assertEqual(r.response.result.status, protos.StatusResult.Success)
            self.assertEqual(
                r.response.return_value, protos.TypedData(json='{"bird": "enarC"}')
            )
    ...
    async def test_mock_activity_trigger_int_to_float(self):
        async with testutils.start_mockhost(
            ...
            await host.init_worker("4.17.1")
            func_id, r = await host.load_function("activity_trigger_int_to_float")
            ...
            _, r = await host.invoke_function(
                ...
                [
                    ...
                    # Activity Trigger's input must be json serializable
                    protos.ParameterBinding(
                        name="input", data=protos.TypedData(json=str(int(10)))
                    )
                ],
            )
            self.assertEqual(r.response.result.status, protos.StatusResult.Success)
            self.assertEqual(r.response.return_value, protos.TypedData(json="-11.0"))
        ...
        async with testutils.start_mockhost(
            ...
            _, r = await host.invoke_function(
                ...
                [
                    protos.ParameterBinding(
                        name="context",
                        data=protos.TypedData(string="Durable functions coming soon"),
                    )
                ],
            )
            self.assertEqual(r.response.result.status, protos.StatusResult.Success)
